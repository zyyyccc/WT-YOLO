...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create cfg ch nc=1 anchors=None
    ...
    if pretrained:
        ...
        if epochs < start_epoch:  # epochs=opt.epochs
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'lmark', 'labels', 'img_size'))
        if RANK in [-1, 0]:
            pbar = tqdm(pbar, total=nb)  # progress bar
        optimizer.zero_grad()

        # img, label_out, self.img_files[index], shapes
        ...
        if RANK in [-1, 0]:
            ...
            if not noval or final_epoch:  # Calculate mAP  # TODO
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/example_dataset.yaml', help='dataset.yaml path')
...
if __name__ == "__main__":
    ...
    end_time = time.time()
    print('耗时:', f'{round((end_time-start_time)/3600, 4)}h')    

...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.img_size = img_size
        self.augment = augment  # train时augment=True
        self.hyp = hyp
...
def load_mosaic(self, index):
    ...
    labels4, segments4 = [], []
    s = self.img_size
    yc, xc = [int(random.uniform(-x, 2 * s + x)) for x in self.mosaic_border]  # mosaic center x, y ; uniform(208, 3*208)
    indices = [index] + random.choices(self.indices, k=3)  # 3 additional image indices
    random.shuffle(indices)
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w) = load_image(self, index)  # img(416,416,3)

        # place img in img4
        if i == 0:  # top left
            img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles ; img4(832,832,3)
    ...
    for x in (labels4[:, 1:], *segments4):
        ...
        x[:, 4:] = np.where(x[:, 4:] < 0, -1, x[:, 4:])
        x[:, 4:] = np.where(x[:, 4:] > 2 * s, -1, x[:, 4:])  
